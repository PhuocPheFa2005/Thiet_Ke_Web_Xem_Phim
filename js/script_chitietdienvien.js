...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let currentPage = 1;
  const movies = document.querySelectorAll(".actors .actor");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const tooltipIMDb = document.getElementById("tooltipIMDb");
  const tooltipAge = document.getElementById("tooltipAge");
  const tooltipDuration = document.getElementById("tooltipDuration");
  const tooltipGenres = document.getElementById("tooltipGenres");
  const watchNowBtn = document.querySelector(".watch-now");

  document.querySelectorAll(".movie").forEach((item, index) => {
      item.addEventListener("mouseenter", function () {
          console.log("Tooltip được kích hoạt"); // Debug xem sự kiện có chạy không
          ...
          if (watchNowBtn) {
              watchNowBtn.onclick = () => {
                  ...
                  window.location.href = `chieuphimle${movieIndex}.html`;
              };
          }
          ...
          if (topPosition + tooltipHeight > window.innerHeight) {
              topPosition = rect.top + window.scrollY - tooltipHeight - spacing;
          }
  ...
  document.addEventListener("mousemove", function (event) {
      if (!tooltip.contains(event.target) && !event.target.closest(".movie")) {
          tooltip.style.display = "none";
      }
  });
});
