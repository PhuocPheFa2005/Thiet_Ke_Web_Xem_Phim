document.addEventListener("DOMContentLoaded", function () {
  ...
  const text =
    "Trải nghiệm xem phim mượt mà, chất lượng cao và hoàn toàn miễn phí!";
  let index = 0;
  ...
  function typeEffect() {
    if (!isDeleting) {
      ...
      if (index > text.length) {
        isDeleting = true;
        setTimeout(typeEffect, 2000); // Đợi 2s trước khi bắt đầu xóa
        return;
      }
    } else {
      textElement.textContent = text.substring(0, index);
    ...
    setTimeout(typeEffect, isDeleting ? 50 : 100); // Tốc độ xóa nhanh hơn gõ
  }
