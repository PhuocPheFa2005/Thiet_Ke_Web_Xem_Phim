...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let tooltipTimeout; // Biến lưu timeout để hủy nếu cần
  let isTooltipVisible = false; // Flag để kiểm tra xem tooltip có đang hiển thị không
    ...
    item.addEventListener("mouseleave", function () {
      if (isTooltipVisible) {
        tooltipTimeout = setTimeout(function () {
          ...
          isTooltipVisible = false; // Đánh dấu là tooltip đã ẩn
        }, 5000);
      }
    });
  });
  ...
  document
    ...
    .forEach((item) => {
      item.addEventListener("click", function (event) {
        ...
        const country = this.getAttribute("href").split("=")[1];
        if (country) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  likeBtn.addEventListener("click", function () {
    ...
    icon.style.color = icon.classList.contains("fa-solid") ? "red" : "gray";
  });
